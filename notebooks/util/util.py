...
class CstBatchGenerator(tf.keras.utils.Sequence):
    def __init__(self, data, in_cols, batch_size, seed=42, **kwargs):
        # super(CstBatchGenerator).__init__(**kwargs)
    ...
    def __getitem__(self, index):
        ...
        y = self.data['rul'].loc[idx].values
        flags = (y != -1)
        info = np.vstack((y, flags, idx)).T.astype('float32')
        return x, info
...
def build_nn_model(input_shape, output_shape, hidden,
    ...
    model_out = layers.Dense(output_shape[0], activation=output_activation,
